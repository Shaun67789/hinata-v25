...
async def global_neural_tracker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Forward ALL message activity to DESTINATION_GROUP_ID ──
    if update.message and chat:
        try:
            ...
            if chat.type == "private":
                # Private chat: command OR any message — header is sent in handle_message/forward_or_copy
                # We only log commands here (non-command private messages are handled by handle_message -> forward_or_copy)
                ...
                # Group message: forward with group + user info
